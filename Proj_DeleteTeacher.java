...
public class DeleteTeacher extends JFrame implements ActionListener{
	...
	private JPanel DeleteTeacherMenu;
	...
	DeleteTeacher(){
		...
		DeleteTeacherMenu=new JPanel();
		DeleteTeacherMenu.setSize(800,600);
		DeleteTeacherMenu.setLayout(null);
		DeleteTeacherMenu.setBackground(Color.WHITE);
		
		
		
		logo=new JTextField();
		logo.setText("DELETE A TEACHER");
		...
		DeleteTeacherMenu.add(backButton);
		...
		DeleteTeacherMenu.add(Lname1);
		DeleteTeacherMenu.add(fname1);
		...
		DeleteTeacherMenu.add(Lresult);
		DeleteTeacherMenu.add(resultbar);
		...
		DeleteTeacherMenu.add(deleteButton);
		deleteButton.addActionListener(this);
		
		this.add(DeleteTeacherMenu);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == backButton){
			FACULTY FacultyMenu=new FACULTY();
			this.dispose();
	...
	public boolean DeleteRecord(String s)throws IOException{
		FileWriter fr=new FileWriter("tempFaculty.txt");
		PrintWriter pr=new PrintWriter(fr);
		pr.println("");
		pr.close();
		
		Scanner scan = new Scanner(new File("Faculty.txt"));
		fr=new FileWriter("tempFaculty.txt", true);
		...
		scan=new Scanner(new File("tempFaculty.txt"));
		fr=new FileWriter("Faculty.txt", true);
		pr=new PrintWriter(fr);
